...
Options:
    ...
    --dest_accuracy_data=<dest_path>    Path to where the results data with 
...
# saving the results data from the modeling script as a local variable to make
# it easier to use
drugs_pred <- read.csv(opt$pred_data_path)

# calculating the accuracy of the classification model
drug_acc <- accuracy(drugs_pred,
                     as.factor(Cannabis),
