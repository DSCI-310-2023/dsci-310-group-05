...
doc <- "This script accesses some of the datasets created in the second script
(pre-processing.R) and create exploratory data visualizations which would
explore the relationship between variables before performing the modeling.
The plots generated are saved within the mentioned directory as png files.
...
cannabis_and_nicotine_graph <- horizontal_hist(processed_data, label, n, "",
...
# Here, we will make a new column called rate to express the percentage of
# each group that uses cannabis, as the data has been standardized and looking
...
openness_and_cannabis_graph <- scatterplot(open_nic_weed,
                                           Oscore,
                                           rate, Nicotine, "Openness",
                                           "Percentage of People Who Use 
                                           Cannabis", "Use Nicotine",
                                           "Openness vs. Cannabis use",
                                           plot_width = 9, plot_height = 7)
ggplot2::ggsave("openness_and_cannabis_graph.png", device = "png",
                path = opt$plot_out_dir, width = 9, height = 9)
