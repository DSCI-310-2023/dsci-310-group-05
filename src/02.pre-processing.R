...
doc <- "This script accesses the dataset downloaded in the first script
(load-data.R).
The dataset is filtered to include samples from those considered as drug users.
The filtered dataset is split into training and testing data based on
user-inputted strata and predictor parameters.
The 'Nicotine' column in the testing dataset is mutated into a numeric variable
where 1 = 'yes' and 0 = 'no'.
The wrangling function is then called on the training data to produce a table
that shows the number individuals within different groups as mentioned as
group_label.


Usage: src/pre-processing.R --file_path=<file_path>
--strata_variable=<strata_variable> --predictor_variable=<predictor_variable>
...
drugs <- select(
  raw_data, Age, Gender, Nscore, Escore, Oscore, Ascore, Nicotine,
  Cannabis
) %>%
  mutate(Cannabis = str_replace_all(Cannabis, c(
    "CL0" = "no", "CL1" = "no",
    "CL2" = "no", "CL3" = "yes",
    "CL4" = "yes", "CL5" = "yes",
    "CL6" = "yes"
  ...
  mutate(Nicotine = str_replace_all(Nicotine, c(
    "CL0" = "no", "CL1" = "no",
    "CL2" = "no", "CL3" = "yes",
    "CL4" = "yes", "CL5" = "yes",
    "CL6" = "yes"
...
testing_data <- mutate(testing_data,
  Nicotine = str_replace_all(
    Nicotine, c("no" = "0", "yes" = "1")
  )
) %>%
  mutate(Nicotine = as.numeric(Nicotine))

group_labels <- c("Neither", "Cannabis only", "Nicotine only", "Both")
cannabis_and_nicotine <- wrangle_training_data(training_data, Nicotine,
                                               Cannabis, group_labels)
...
utils::write.csv(cannabis_and_nicotine, opt$dest_processed_data,
                 row.names = FALSE)
