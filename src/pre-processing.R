...
doc <- "This script accesses the dataset downloaded in the first script (load-data.R). 
The dataset is filtered to include samples from those considered as drug users. 
The filtered dataset is split into training and testing data based on user-inputted strata and predictor parameters. 
The 'Nicotine' column in the testing dataset is mutated into a numeric variable where 1 = 'yes' and 0 = 'no'. 
The wrangling function is then called on the training data to produce a table that shows the number individuals within different groups as mentioned as group_label.
