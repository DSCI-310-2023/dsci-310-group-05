...
raw_data <- read_csv("../data/drug_consumption.csv")
knitr::kable(head(raw_data), caption = "Summary table of the entire dataset")
```
...
drugs <- read_csv("../data/drug_user_consumption.csv")
knitr::kable(head(drugs), caption = "Summary table of the dataset used in the analysis")
```
...
``` {r cannabis_and_nicotine-table, echo = FALSE,  message=FALSE, warning=FALSE}
cannabis_and_nicotine <- read_csv("../data/processed_training_data.csv")
knitr::kable(head(cannabis_and_nicotine), caption = "Table of the number of individuals (n) that fall into each label")
```
...
```{r cannabis-nicotine, echo = FALSE,out.width="50%", fig.cap = "Horizontal histogram of Nicotine usage vs Cannabis usage"}
...
```{r cannabis-graph, echo = FALSE, out.width="50%", out.height="50%", fig.cap = "Scatter plot of cannabis use % vs openness. Openness is the standardized personality Oscore."}
...
``` {r drug_data-table, echo = FALSE,  message=FALSE, warning=FALSE}
drug_data <- read_csv("../data/standardized_training_data.csv")
knitr::kable(head(drug_data), caption = "Table of used predictors with Nicotine standardized, where Nicotine = 1 indicates nicotine use and Nicotine = 0 indicates no nicotine use.")
```
...
drugs_workflow <- read_csv("../data/drugs_workflow_data.csv")
knitr::kable(head(drugs_workflow), caption = "Table of accuracy estimates and standard error outputs corresponding to a few different values of K-neighbours.")
```
...
```{r acc-plot, echo = FALSE, fig.cap = "Plot of accuracy estimate versus number of neighbours. Mean accuracy in % for different values of K-neighbours. K = 23 and K = 24 yield the highest accuracies, and after K exceeds 24, accuracy starts to decline."}
...
drugs_pred <- read_csv("../data/results_data.csv")
